...
import logging
from utils.ncaab_teams import get_ncaab_team_by_abbr

logger = logging.getLogger(__name__)

class NCAABSport(BaseSport):
    def __init__(self, api_url: str, db_path: str = "sports_data.db"):
        super().__init__("ncaab", api_url, db_path)
    
    def get_team_abbr(self, team_name: str) -> str:
        if not team_name:
            return 'N/A'
        # Try to find in NCAAB teams list
        team = get_ncaab_team_by_abbr(team_name)
        if team:
            return team['abbr']
        # Try to find by name
        from utils.ncaab_teams import NCAAB_TEAMS
        for t in NCAAB_TEAMS:
            if t['name'].lower() in team_name.lower() or team_name.lower() in t['name'].lower():
                return t['abbr']
        return team_name[:3].upper()
    
    def parse_gamelines(self, data: Dict) -> List[Dict]:
        """Parse NCAAB gameline data"""
        games = []
    ...
    def _parse_games(self, games_data: List[Dict], source: str) -> List[Dict]:
        ...
        for game in games_data:
            ...
            games.append({
                'game_id': game.get('game_id', f"ncaab_{len(games)}"),
                'source': source,
                'home_team': home_team,
                'away_team': away_team,
    ...
    async def get_team_stats(self, team: str, year: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error fetching NCAAB {team} stats: {e}")
            return []